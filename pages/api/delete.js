        ...
        if(incomingMediaUrl) {
          ...
          res.send(`<Response><Message>Transcription: ${transcription}</Message></Response>`);
            ...
            if(gpt3Response.length < 1500) {
                ...
                res.status(204).end();
                await sendTwilioMessage1600Characters(gpt3Response, fromNumber);
            }
        }
    }
  } catch(err) {
...
async function getGpt4Response(prompt) {
  try {
    ...
    console.error(`Failed to get GPT-4 response: ${err}`);
    throw err;
  }
}
...
function splitMessage(message, limit) {
  ...
  while (message.length > 0) {
      let chunk = message.substr(0, limit);
...
async function sendTwilioMessage1600Characters(gpt4Response, toNumber) {
  try {
    ...
    console.error(`Failed to send message with 1600 characters: ${err}`);
    throw err;
  }
}
